...
export function BuildTable(name: string, dataRows: Record<string, string | number | boolean | null>[] | undefined): string {
  const container = (value: string) => `<div class="dataContainer"><h3>${escapeHtml(name)}</h3>${value}</div>`;
  ...
  return container(`<table class="dataTable"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table>`);
}
